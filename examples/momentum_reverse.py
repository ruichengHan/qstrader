...
class TopNMomentumAlphaModel(AlphaModel):
    ...
    def _highest_momentum_asset(
        ...
        """
        assets = self.signals['vol'].assets

        # Calculate the holding-period return momenta for each asset,
        # for the particular provided momentum lookback period
        all_momenta = {
            asset: self.signals['vol'](
                asset, self.mom_lookback
            ) for asset in assets
        }

        all_momenta = dict(list(filter(lambda x: x[1] > 0, all_momenta.items())))

        return [asset[0] for asset in sorted(all_momenta.items(), key=operator.itemgetter(1))][:self.mom_top_n]
...
if __name__ == "__main__":
    ...
    volatility = VolatilitySignal(start_dt, strategy_universe, lookbacks=[mom_lookback])
    signals = SignalsCollection({'momentum': momentum, "vol": volatility}, strategy_data_handler)
    ...
    tearsheet = TearsheetStatistics(
        ...
        title='US Sector Momentum - Top 10 Sectors'
    )
